Feature: Update specific field

  @sanity
  Scenario: Check if Admin is able to update a user with all valid credentials
    Given Admin set the PATCH request with the valid request body and valid Endpoint
  ...
  Scenario: Check if Admin is able to update a user with invalid content Type
    Given Admin set the PATCH request with the invalid contentType
  ...
  Scenario: Check if Admin is able to update a user with No Auth
    Given Admin set the PATCH request with the valid request body with no Auth
  ...
  Scenario: Check if Admin is able to update a user with invalid basic auth
    Given Admin set the PATCH request with the valid request body with invalid basic Auth
    When Admin sends HTTPS Request and update request Body with no or invalid auth
    Then Admin receives "401" "Unauthorized" Status for update

  Scenario: Check if User is able to update new User with invalid User ID
    Given Admin set the PATCH request with Valid Endpoint and the request Body is missing the mandatory fields
  ...
  Scenario: Check if Admin is able to update a user with first name as numeric
    Given Admin set the PATCH request
